...
public class JoinRoomNum : MonoBehaviour {
	...
	void Start () {
        RoomNum = this.transform.Find("RoomNum").transform;
        NumGrid = this.transform.Find("NumGrid").transform; 
        //给数字按钮添加点击事件
        foreach (Transform item in NumGrid.transform)
        {            
            item.GetComponent<Button>().onClick.AddListener(()=> {
                OnNumBtnClick(item.gameObject);
            });
        }
	}

    public void OnNumBtnClick(GameObject item)
    {
        //获取当前点击的物体
        AudioManager.Instance.PlayEffectAudio("ui_click"); 
        string str= item.name.Substring(7, 1);
        switch (str)
        {
            case "0": 
